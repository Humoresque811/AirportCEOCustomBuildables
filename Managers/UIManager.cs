...
class UIManager : MonoBehaviour
{
    ...
    public static void CreateAllUI()
    {
        if (FloorModSourceCreator.Instance.buildableMods.Count > 0)
        {
            ...
            foreach (TexturedBuildableMod buildableMod in FloorModSourceCreator.Instance.buildableMods)
            {
                ...
                if (floorMod == null)
                {
                    AirportCEOCustomBuildables.LogError($"A buildable mod in FloorModSourceCreator is not a buildable mod!");
                    continue;
                }
        ...
        foreach (Type type in FileManager.Instance.buildableTypes.Keys)
        {
            ...
            {
                AirportCEOCustomBuildables.LogError($"There are not the same amount of buildable mods as buildables in type {type.Name}. Skipped loading");
                continue;
            }
    ...
    private static void CreateUI(TexturedBuildableMod buildableMod, GameObject buildable, Type type)
    {
        try
        {
            ...
            {
                AirportCEOCustomBuildables.LogError("UI icon creators type not an expected type!");
            }
            AirportCEOCustomBuildables.LogInfo("[Success] Created UI button \"" + buildableMod.name + "\" successfully");
        }
        catch (Exception ex)
        {
            AirportCEOCustomBuildables.LogError($"Creating UI button \"{buildableMod.name}\" failed. {ExceptionUtils.ProccessException(ex)}");
        }
