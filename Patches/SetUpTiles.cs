...
class SetUpTiles
{
    public static void Prefix(TiledObjectsManager __instance, ITileable tileable)
    {
        ...
        LoadSubSystem.LoadedWrapper = LoadSubSystem.GetCustomSaveData();

        if (LoadSubSystem.LoadedWrapper == null)
        {
            AirportCEOCustomBuildables.Log("[Loading Issue] Loaded wrapper was null, so the setTile system can't set tiles! (If you see this error, then " +
        ...
        foreach (CustomFloorSerializable customFloor in LoadSubSystem.LoadedWrapper.customFloorSerializables)
        {
            ...
            for (int i = 0; i < FloorModSourceCreator.Instance.buildableMods.Count; i++)
            {
                ...
                if (!string.Equals(floorMod.id, customFloor.modId))
                {
                    ...
                    SaveLoadSystem.Quicklog("[Buildable Problem] A custom floor is in the processs of being loaded, but a custom floor with the id in the save file doesn't exist. " +
