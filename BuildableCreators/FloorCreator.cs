...
class FloorCreator : MonoBehaviour, IBuildableCreator
{
    ...
    public void CreateBuildables()
    {
        ...
        for (int i = 0; i < FileManager.Instance.buildableTypes[typeof(FloorMod)].Item2.buildableMods.Count; i++) 
        {
            ...
            if (floorMod == null)
            {
                AirportCEOCustomBuildables.LogError($"A buildable mod of FloorCreators is not an floor mod!");
                continue;
            }
            ...
            try
            {
                ...
                {
                    AirportCEOCustomBuildables.LogError($"Failed to get PlaceableFloor component of floor!");
                    continue;
                }
                ...
                {
                    AirportCEOCustomBuildables.LogError($"Failed to get SpriteRenderer component of floor!");
                    continue;
                }
                ...
                AirportCEOCustomBuildables.LogInfo($"[Success] Created buildable floor \"{floorMod.name}\" successfully!");

                newFloor = null;
            }
            catch (Exception ex)
            {
                AirportCEOCustomBuildables.LogError($"Creating floor \"{floorMod.name}\" failed. {ExceptionUtils.ProccessException(ex)}");
            }
        }
    }
}
