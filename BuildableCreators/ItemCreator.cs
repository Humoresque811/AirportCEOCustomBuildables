...
class ItemCreator : MonoBehaviour, IBuildableCreator
{
    ...
    public void CreateBuildables()
    {
        ...
        for (int i = 0; i < FileManager.Instance.buildableTypes[typeof(ItemMod)].Item2.buildableMods.Count; i++)
        {
            ...
            if (itemMod == null)
            {
                AirportCEOCustomBuildables.LogError($"A buildable mod of ItemSourceCreators is not an item mod!");
                continue;
            }
            ...
            try
            {
                ...
                {
                    AirportCEOCustomBuildables.LogError($"New Buildable \"{itemMod.name}\" is null!");
                    continue;
                }
                ...
                AirportCEOCustomBuildables.LogInfo($"[Success] Created buildable item \"{itemMod.name}\" successfully");

                newBuildable = null;
            }
            catch (Exception ex)
            {
                AirportCEOCustomBuildables.LogError($"Creating buildable \"{itemMod.name}\" failed. {ExceptionUtils.ProccessException(ex)}");
                itemsFailed = true;
            }
        }
    ...
    public void ConvertItemToCustom(GameObject item, int index, bool useRandomRotation, float spriteRotation, float itemRotation)
    {
        ...
        try
        {
            ...
            {
                AirportCEOCustomBuildables.LogError($"Item provided to convertItemToCustom is null or the index provided is less than one!");
                return;
            }
        ...
        {
            AirportCEOCustomBuildables.LogError($"An error occured while converting an item to custom. Info:" +
                "\n " + ExceptionUtils.ProccessException(ex) + 
    ...
    private GameObject ConvertPlaceableItemIntoCustom(GameObject item, ItemMod itemMod, int index, float itemRotation)
    {
        ...
        {
            AirportCEOCustomBuildables.LogError($"There is no shadow handler...? Item: " + itemMod.name);
        }
        return item;
    }
