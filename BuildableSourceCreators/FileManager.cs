...
class FileManager : MonoBehaviour
{
    ...
    public void SetUp()
    {
        AirportCEOCustomBuildables.LogInfo("[Init] FileManager is being setup!");
        Instance = this;
    }

    public void SetUpBuildableTypes()
    {
        AirportCEOCustomBuildables.LogInfo("[Init] Setting up Custom Buidables types!");
    ...
    {
        AirportCEOCustomBuildables.LogInfo("[Init] Starting basepath initilization!");
        try
        {
            string basePath = Path.Combine(Application.persistentDataPath, pathAddativeBase);
            if (!Directory.Exists(basePath))
            {
                AirportCEOCustomBuildables.LogInfo("Basepath does not exist. Creating folder!");
            ...
            foreach (Type type in buildableTypes.Keys.ToArray())
            {
                ...
                {
                    AirportCEOCustomBuildables.LogInfo($"The folder for buildable type \"{buildableTypes[type].Item1}\" does not exist. Creating folder!");
                    Utils.CreateFolderIfNotExist(extendedPath);
                }
            }

            AirportCEOCustomBuildables.LogInfo("[Success] Got/Created buildable folders");
        }
        catch (Exception ex)
        {
            AirportCEOCustomBuildables.LogError($"Error creating basepaths! {ExceptionUtils.ProccessException(ex)}");
        }
    }
    ...
    private bool GetTextureSpriteFromPath(TexturedBuildableMod buildableMod, string spriteType, out Sprite sprite, int pixelsPerUnit = 256)
    {
        ...
        if (string.IsNullOrEmpty(buildableMod.pathToUse))
        {
            ...
            {
                AirportCEOCustomBuildables.LogError($"Buildable mod to get sprite for is not a valid type (being {buildableMod.GetType()}).");
                sprite = null;
                return false;
            }
        ...
        else
        {
            ...
            AirportCEOCustomBuildables.LogError("Invalid image type input into the image loader.");
            sprite = Singleton<DataPlaceholderItems>.Instance.smallPlantIcon;
            return false;
        }
    ...
    public bool GetTexture2DFromPath(TexturedBuildableMod buildableMod, out Texture2D texture2D)
    {
        ...
        if (string.IsNullOrEmpty(buildableMod.pathToUse))
        {
            ...
            if (String.IsNullOrEmpty(pathAddative))
            {
                AirportCEOCustomBuildables.LogError($"Buildable mod to get texture2D for is not a valid type (being {buildableMod.GetType()}).");
                texture2D = null;
                return false;
            }
    ...
    public string GetJSONFileContent(string path)
    {
        if (!File.Exists(path))
        {
            AirportCEOCustomBuildables.LogError("Nothing at GetJSONFileContent's provided search path!");
            return null;
        }
    ...
    public bool GetCorrectPath(ref string path, Type modtype, Action<string> logger)
    {
        if (string.IsNullOrEmpty(path))
        {
            ...
            logger.Invoke("Started reading the LocalLow buildables folder");
            return false;
        }
        else
        {
            logger.Invoke("Started reading a folder (Workshop or Native mod folder)");
            return true;
        }
    }
    ...
    public bool GetDirectories(string path, out string[] directories, Action<string> logger)
    {
        ...
        {
            logger.Invoke("[Error via Logger] Couldn't get JSON file paths... Error: " + ex.ToString());
            directories = null;
            return false;
        }
    }
    ...
    public void CrossCheckIds(Action<string> logger)
    {
        foreach (Type type in buildableTypes.Keys)
        {
            ...
            for (int i = 0; i < buildableMods.Count; i++)
            {
                ...
                DialogUtils.QueueDialog($"[Airport CEO Custom Buildables] You have two mods with the same id.. This will result in problems with saveload, so the mod " +
                    $"called {BuildableClassHelper.GetModIdentification(buildableMods[i])} was removed");
